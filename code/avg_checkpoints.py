...
flags.DEFINE_string("prefix", "../model/en-to-fr_L-4_H-128_A-8_DFF-512_V-30k_back_translation/checkpoint",
                    "Prefix (e.g., directory) to append to each checkpoint.")
flags.DEFINE_string("output_path", "../model/averaged.ckpt",
...
def main(_):
    # harman: disable eager execution
    ...
    for checkpoint in checkpoints:
        ...
        for name in var_values:
            ...
            # harman: try catch was added to fix tensor.dtype error for bytes for key: _CHECKPOINTABLE_OBJECT_GRAPH
            try:
                var_dtypes[name] = tensor.dtype
                var_values[name] += tensor
            except:
                # will drop keys
                keys_to_drop.append(name)
        # harman: dropping problematic key
        for key in keys_to_drop:
            var_values.pop(key, None)
